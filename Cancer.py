...
def  model(X_train,y_train,X_test,y_test,n_h,num_iters,alpha,print_cost):
    np.random.seed(3)
    
    n_x = layers(X_train,y_train)[0]
    n_y = layers(X_train, y_train)[1]
    ...
    for i in range(0,num_iters):
        ...
        grads = back_prop(parameters, cache, X_train,y_train)       
...
parameters = model(X_train.T, y_train.T,X_test.T,y_test.T, n_h=20, num_iters=5000, alpha=0.0075, print_cost=True)
